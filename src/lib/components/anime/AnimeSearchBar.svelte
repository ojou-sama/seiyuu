<script lang="ts">
	...
	type Props = {
		...
		disabled?: boolean;
		searchFunction: (query: string) => Promise<Anime[]>;
		fetchItemFunction: (id: number) => Promise<Anime>;
	};

	const { onSelect, disabled = false, searchFunction, fetchItemFunction }: Props = $props();
	...
	async function handleSelect(anime: Anime) {
		try {
			const fullAnime = await fetchItemFunction(anime.id);
			onSelect?.(fullAnime);
			inputValue = '';
			results = [];
			isOpen = false;
